...
class TestSuite(ABC):
    ...
    @abstractmethod
    async def add_test(self, test_def: TestCaseInfo) -> None:
        pass
    ...
    async def _test_defs(self, test_list: List[str]) -> List[TestCaseInfo]:
        """For the tests of the suite, build a list (test, case)"""
        # If not implemented in subclass, just return the list of tests
        return [TestCaseInfo(self.name, test_name, None) for test_name in test_list]
    ...
    async def add_test_list(self) -> None:
        ...
        patterns = options.name if options.name else [self.name]

        is_disabled = lambda x: str(x) in self.disabled_tests
        should_skip = lambda x: options.skip_pattern and options.skip_pattern in str(x)

        pending = []
        for test_def in test_defs:
            for _ in range(options.repeat):
                if not is_disabled(test_def):
                    if not should_skip(test_def):
                        if any(p in str(test_def) for p in patterns):
                            pending.append(asyncio.create_task(self.add_test(test_def)))
...
class UnitTestSuite(TestSuite):
    ...
    async def create_test(self, test_def: TestCaseInfo, suite: TestSuite, args) -> None:
...
class BoostTestSuite(UnitTestSuite):
    ...
    async def _test_defs(self, test_list: List[str]) -> List[TestCaseInfo]:
        ...
        for test_name in test_list:
            ...
            ret.extend([TestCaseInfo(self.name, test_name, case) for case in case_list])
...
class Test:
    ...
    def __init__(self, test_no: int, test_def: TestCaseInfo, suite: TestSuite) -> None:
        self.id = test_no
        self.path = ""
        self.args: List[str] = []
        ...
        self.full_name = f"{test_def.name}::{test_def.case}"
        self.full_name_dot = f"{test_def.name}.{test_def.case}"
        self.mode = suite.mode
        self.suite = suite
        # Unique file name, which is also readable by human, as filename prefix
        # TODO: change inner '.' to '-' for a more conventional file naming
        self.uname = f"{self.suite.name}.{self.full_name_dot}.{self.id}"
...
class UnitTest(Test):
    ...
    def __init__(self, test_no: int, test_def: TestCaseInfo, suite, args: str) -> None:
...
class BoostTest(UnitTest):
    ...
    def __init__(self, test_no: int, test_def: TestCaseInfo, suite, args: str,
...
class CQLApprovalTest(Test):
    ...
    def __init__(self, test_no: int, test_def: TestCaseInfo, suite) -> None:
    ...
    for suite in TestSuite.suites.values():
        for test in suite.junit_tests():
            ...
            xml_res = ET.SubElement(xml_results, 'testcase',
                                    name=f"{test.suite.name}.{test.full_name_dot}.{mode}.{test.id}")
