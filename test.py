...
class TestSuite(ABC):
    ...
    async def add_test_list(self) -> None:
        ...
        for test_file in lst:
            ...
            async def add_test(test_file: str) -> None:
            ...
            if not is_disabled(test_file) and not should_skip(t) and is_enabled(t):
...
class CQLApprovalTest(Test):
    ...
    def __init__(self, test_no: int, test_file: str, suite) -> None:
        ...
        self.path = "pytest"
        self.cql = suite.suite_path / (self.test_file + ".cql")
        self.result = suite.suite_path / (self.test_file + ".result")
        self.tmpfile = os.path.join(suite.options.tmpdir, self.mode, self.uname + ".reject")
        self.reject = suite.suite_path / (self.test_file + ".reject")
    ...
    for suite in TestSuite.suites.values():
        for test in suite.junit_tests():
            ...
            xml_res = ET.SubElement(xml_results, 'testcase',
                                    name="{}.{}.{}.{}".format(test.suite.name, test.test_file, mode, test.id))
