...
class TestSuite(ABC):
    ...
    def remaining_first(self, remaining_first: int):
        self._remaining = remaining_first
        self._update_run_priority()
    ...
    def _update_run_priority(self):
        ...
        self.precedence = (self.workers * self.mode_idx, -self.remaining_first * self.mode_idx,
    ...
    async def run(self, lock: asyncio.Lock, console: "TabularConsoleOutput", 
                  memory_manager: "MemoryManager", **test_args) -> None:
        ...
        if self.min_memory > memory_manager.mb_available or \
                not await memory_manager.reserve(self.min_memory, lambda: self.remaining > 0):
            logger.error("Not enough memory to run suite")
            # XXX mark all tests failed/skipped?
            async with lock:
        ...
        while True:
            ...
            # Reserve
            if test.shortname in extra_memory:
                res = await memory_manager.reserve(self.min_memory, lambda: self.remaining > 0)
                if not res:
                    async with lock:
                        if self.runners == 1:
                            # Skip all tests
...
class MemoryManager:
    ...
    """
    def __init__(self, lock: asyncio.Lock, base_delay: float = 0.1, max_delay: float = 100.):
        ...
        while True:
            async with self.lock:
                ...
                if mb_required < (self.mb_available + self.mb_reserved):
                    ...
                    if mb_required < psutil.virtual_memory().mb_available / 1024 ** 2:
        ...
        async def run(self):
            ...
            while True:
                try:
                    ...
                    await suite.run(worker_lock, memory_manager, console)   # START TEST RUNNER
                    async with worker_lock:
                        suite.workers -= 1
