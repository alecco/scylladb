...
class TestSuite(ABC):
    ...
    def __init__(self, path: str, cfg: dict, options: argparse.Namespace, mode: str) -> None:
        ...
        self.test_memory = collections.defaultdict(lambda: 0.0, self.cfg.get("test_memory", {}))
        self.test_memory["default"] = self.test_memory.get("default", 0)  # unspecified
            ...
            try:
                for i in range(1, self.FLAKY_RETRIES):
                    if i > 1:
                        ...
                        test.reset()
                    print(f"XXX {worker_name} {runner_id} TestSuite.run: {test.uname}")
    ...
    async def add_test_list(self) -> None:
        ...
        for test in self.tests:
            test.test_memory = self.test_memory[test.shortname]
...
class MemoryManager:
    ...
    """
    def __init__(self, lock: asyncio.Lock, base_delay: float = 0.1, max_delay: float = 30.):
    ...
    async def reserve(self, mb_required: int, check: Callable[[], bool] = lambda: True) -> bool:
        ...
        if actual < expected * .8:
            print(f"\n\nXXX MemoryManager() total available {self.mb_available:.02f} - reserved {expected:.02f} vs. actual {actual:02f} <<<<<<<<<\n")

        if mb_required == 0:
            return True
        # print(f"XXX MemoryManager(mb_required={mb_required}) > {self.mb_available - self.mb_reserved:.02f} available <<")
