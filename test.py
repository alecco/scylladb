...
def update_stats_db(stats_db: sqlite3.Connection, test: Test) -> None:
    ...
    print(f"XXX {test.name}")
    sample_data = (test.name, "mycase", 5, 123.45)  # 5 seconds as an example
    cursor = stats_db.cursor()
    res = cursor.execute('INSERT INTO test_stats (test, "case", seconds, memory) VALUES (?, ?, ?, ?)', sample_data)
    print(f"XXX re {res}")
    cursor.close()
...
async def run_all_tests(signaled: asyncio.Event, options: argparse.Namespace) -> None:
    ...
    try:
        ...
        if stats_db:
            # XXX stats_db.execute('PRAGMA wal_checkpoint(FULL)')
            stats_db.close()
