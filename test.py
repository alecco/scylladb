...
class TestSuite(ABC):
    ...
    @abstractmethod
    async def add_test(self, file_name: str) -> None:
        pass
    ...
    async def add_test_list(self) -> None:
        ...
        for test_name in lst:
            ...
            t = os.path.join(self.name, test_name)

            async def add_test(test_name) -> None:
            ...
            if not is_disabled(test_name) and not should_skip(t) and is_enabled(t):
                add_test_tasks.append(asyncio.create_task(add_test(test_name)))
...
class UnitTestSuite(TestSuite):
    ...
    async def create_test(self, test_file: str, suite: TestSuite, args):
        exe = os.path.join("build", suite.mode, "test", suite.name, test_file)
        if not os.access(exe, os.X_OK):
            print(palette.warn(f"Unit test executable {exe} not found."))
            return
        test = UnitTest(self.next_id((test_file, self.suite_key)), test_file, suite, args)
        self.tests.append(test)

    async def add_test(self, test_file: str) -> None:
        ...
        if os.path.join("test", self.name, test_file) not in self.options.tests:
            return

        # Default seastar arguments, if not provided in custom test options,
        # are two cores and 2G of RAM
        args = self.custom_args.get(test_file, ["-c2 -m2G"])
        for a in args:
            await self.create_test(test_file, self, a)
...
class BoostTestSuite(UnitTestSuite):
    ...
    async def create_test(self, test_file: str, suite, args) -> None:
        exe = os.path.join("build", suite.mode, "test", suite.name, test_file)
        if not os.access(exe, os.X_OK):
        ...
        allows_compaction_groups = self.all_can_run_compaction_groups_except != None and \
                                   test_file not in self.all_can_run_compaction_groups_except
        if options.parallel_cases and (test_file not in self.no_parallel_cases):
            ...
            if len(case_list) == 1:
                test = BoostTest(self.next_id((test_file, self.suite_key)), test_file, suite,
                                 args, None, allows_compaction_groups)
                self.tests.append(test)
            else:
                for case in case_list:
                    test = BoostTest(self.next_id((test_file, self.suite_key, case)), test_file,
                                     suite, args, case, allows_compaction_groups)
                    self.tests.append(test)
        else:
            test = BoostTest(self.next_id((test_file, self.suite_key)), test_file, suite, args,
...
class PythonTestSuite(TestSuite):
    ...
    async def add_test(self, test_file: str) -> None:
        test = PythonTest(self.next_id((test_file, self.suite_key)), test_file, self)
        self.tests.append(test)
...
class CQLApprovalTestSuite(PythonTestSuite):
    ...
    async def add_test(self, test_file: str) -> None:
        test = CQLApprovalTest(self.next_id((test_file, self.suite_key)), test_file, self)
        self.tests.append(test)
...
class TopologyTestSuite(PythonTestSuite):
    ...
    async def add_test(self, test_file: str) -> None:
        """Add test to suite"""
        test = TopologyTest(self.next_id((test_file, 'topology', self.mode)), test_file, self)
        self.tests.append(test)
...
class RunTestSuite(TestSuite):
    ...
    async def add_test(self, test_file: str) -> None:
        test = RunTest(self.next_id((test_file, self.suite_key)), test_file, self)
        self.tests.append(test)
...
class Test:
    ...
    def __init__(self, test_no: int, test_file: str, suite) -> None:
        self.id = test_no
        self.path = ""
        self.args: List[str] = []
        self.valid_exit_codes = [0]
        # Test file name with test suite name
        self.name = os.path.join(suite.name, test_file.split('.')[0])
        # Test file name within the suite
        self.test_file = test_file
        self.mode = suite.mode
        self.suite = suite
        # Unique file name, which is also readable by human, as filename prefix
        self.uname = "{}.{}.{}".format(self.suite.name, self.test_file, self.id)
        ...
        self.is_flaky = self.test_file in suite.flaky_tests
...
class UnitTest(Test):
    ...
    def __init__(self, test_no: int, test_file: str, suite, args: str) -> None:
        super().__init__(test_no, test_file, suite)
...
class BoostTest(UnitTest):
    ...
    def __init__(self, test_no: int, test_file: str, suite, args: str,
                 casename: Optional[str], allows_compaction_groups : bool) -> None:
        boost_args = []
        if casename:
            name = f"{test_file}.{casename}"
            boost_args += ['--run_test=' + casename]
        else:
            name = test_file
        super().__init__(test_no, name, suite, args)
...
class RunTest(Test):
    ...
    def __init__(self, test_no: int, test_file: str, suite) -> None:
        super().__init__(test_no, test_file, suite)
        self.path = suite.suite_path / test_file
        self.xmlout = os.path.join(suite.options.tmpdir, self.mode, "xml", self.uname + ".xunit.xml")
...
class PythonTest(Test):
    ...
    def _prepare_pytest_params(self, options: argparse.Namespace):
        ...
        self.args.append(str(self.suite.suite_path / (self.test_file + ".py")))
    ...
    async def run(self, options: argparse.Namespace) -> Test:
        ...
        try:
            ...
            if self.test_file in self.suite.dirties_cluster:
