...
class TestTaskDef(NamedTuple):
    ...
    def startswith(self, test_file: str, case_name: Optional[str]):
        """Checks if self's test_file string starts with provided test_file string.
           If case_name is not provided returns True. (matches all cases of the test_file provided)
...
class BoostTestSuite(UnitTestSuite):
    ...
    async def _test_defs(self, test_list: List[str]) -> List[TestTaskDef]:
        ...
        if not self.options.parallel_cases:
            ...
            return [TestTaskDef(self.name, test_file, None) for test_file in test_list]
    ...
    async def create_test(self, test_def: TestTaskDef, suite: TestSuite, args) -> None:
        ...
        test = BoostTest(self.next_id((test_def.test_file, self.suite_key, test_def.case_name)), test_def, suite, args, allows_compaction_groups)
        self.tests.append(test)
...
class PythonTestSuite(TestSuite):
    ...
    async def _test_defs(self, test_list: List[str]) -> List[TestTaskDef]:
        ...
        tasks = [self._create_test_case(test_file) for test_file in test_list]
...
class CQLApprovalTestSuite(PythonTestSuite):
    ...
    async def add_test(self, test_def: TestTaskDef) -> None:
        assert test_def.case_name is None
...
class TopologyTestSuite(PythonTestSuite):
    ...
    async def add_test(self, test_def: TestTaskDef) -> None:
        """Add test to suite"""
        test = TopologyTest(self.next_id((test_def.test_file, test_def.case_name, 'topology', self.mode)), test_def, self)
        self.tests.append(test)
...
class RunTestSuite(TestSuite):
    ...
    async def add_test(self, test_def: TestTaskDef) -> None:
        assert test_def.case_name is None
        test = RunTest(self.next_id((test_def.test_file, test_def.case_name, self.suite_key)), test_def, self)
        self.tests.append(test)
...
class Test:
    ...
    def __init__(self, test_no: int, test_def: TestTaskDef, suite: TestSuite) -> None:
        ...
        test_file_no_ext = test_def.test_file.split('.')[0]
        ...
        if test_def.case_name is None:
            ...
            self.is_flaky = f"{self.name}::{self.case_name}" in suite.flaky_tests
...
class CQLApprovalTest(Test):
    ...
    def __init__(self, test_no: int, test_def: TestTaskDef, suite) -> None:
        assert test_def.case_name is None, "CQLApprovalTest does not have test cases inside a test"
        super().__init__(test_no, test_def, suite)
...
class PythonTest(Test):
    ...
    def __init__(self, test_no: int, test_def: TestTaskDef, suite) -> None:
        ...
        self.xmlout = os.path.join(self.suite.options.tmpdir, self.mode, "xml",
                                   f"{self.test_def.test_file.split('.')[0]}{case_name}.xunit.xml")
        self.server_log: Optional[str] = None
        self.server_log_filename: Optional[pathlib.Path] = None
    ...
    def _prepare_pytest_params(self, options: argparse.Namespace):
        ...
        t = f"{self.test_def.test_file}{'::' + self.case_name if self.case_name else ''}"
        self.args.append(t)
