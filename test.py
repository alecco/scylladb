...
class CQLApprovalTest(Test):
    ...
    async def run(self, options: argparse.Namespace) -> Test:
        ...
        try:
            ...
            self.args.insert(1, f"--harness-api={self.harness.sock_path}")
...
class PythonTest(Test):
    ...
    async def run(self, options: argparse.Namespace) -> Test:
        ...
        try:
            await self.harness.start()
            self.args.insert(0, "--host={}".format(self.harness.cluster[0].host))  # Legacy
            self.args.insert(0, f"--harness-api={self.harness.sock_path}")
