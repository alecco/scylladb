...
class TestDef(NamedTuple):
    """A test and case definition with suite, test, and case.
       If case is None, it represents the whole test (file).
       ...
       Representation is  suite::test[::case] where case is ommitted if not present.
    ...
    def __str__(self):
        """Represent this TestDef as suite::test::case
           If suite is not specified, 'all::test::case' or 'all::test'.
           If test is not specified, 'suite::'.
           If case is not specified, 'suite::test'.
        """
        ...
        assert self.test_file or (not self.test_file and not self.case), "No case if test is undefined"
        test_name = self.test_file if self.test_file else ""
        case_ = f"::{self.case}" if self.case else ""
        return f"{suite}{test_name}{case_}"

    def match(self, other: "TestDef") -> bool:
        """
        Check if the 'other' TestDef matches the pattern specified by this TestDef instance.
        ...
        if self.test_file is not None and other.test_file is not None and \
            ...
            return False
        if self.case is not None and other.case is not None and not self.case in other.case:
            return False
        return True

    def startswith(self, test_file: str, case: Optional[str]):
        """Checks if self's test_file string starts with provided test_file string.
           If case is not provided returns True. (matches all cases of the test_file provided)
           If case is not provided and self has it defined, returns False.
           If case is provided but not definded in self, returns False.
           If case is provided and definded in self, and self starts with provided, returns True.
        """
        assert self.test_file, "Need to have test_file to compare"
        return self.test_file.startswith(test_file) and \
                (not case or (self.case is not None and self.case.startswith(case)))
...
class TestSuite(ABC):
    ...
    @abstractmethod
    async def add_test(self, test_def: TestDef) -> None:
        pass
    ...
    async def _test_defs(self, test_list: List[str]) -> List[TestDef]:
        """From a list of test names (files) of this suite, build a list of TestDef definitions
           containing the suite name, the (provided) test name, and test cases.
           If there are no cases, just return a list of one TestDef with case = None.
        """
        # If not implemented in subclass, just return the list of tests
        return [TestDef(self.name, test_name, None) for test_name in test_list]
    ...
    async def add_test_list(self) -> None:
        ...
        if test_defs and self.run_first_tests:
            ...
            test_defs.sort(key=lambda tc: not any(tc.startswith(name, case)
                                                  for name, case in run_first_tc))

        def is_enabled(test_def: TestDef):
            if not options.names:
                return True             # no test specified in command line, run all tests
            # TODO: options.names is actually List[TestDef]
...
class UnitTestSuite(TestSuite):
    ...
    async def create_test(self, test_def: TestDef, suite: TestSuite, args) -> None:
        exe = os.path.join("build", suite.mode, "test", suite.name, test_def.test_file)
        if not os.access(exe, os.X_OK):
            print(palette.warn(f"Unit test executable {exe} not found."))
            logging.warning(f"Unit test executable {exe} not found.")
            return
        test = UnitTest(self.next_id((test_def.test_file, self.suite_key)), test_def, suite, args)
    ...
    async def add_test(self, test_def: TestDef) -> None:
        ...
        if os.path.join("test", self.name, test_def.test_file) not in self.options.tests:
            return
        ...
        args = self.custom_args.get(test_def.test_file, ["-c2 -m2G"])
        for a in args:
...
class BoostTestSuite(UnitTestSuite):
    ...
    async def _test_defs(self, test_list: List[str]) -> List[TestDef]:
        """From a list of tests (files) get the cases for each one and return a list of TestDef
           definitions with this suite's name, the test name provided, and each of the case names
           found.
        """

        if not self.options.parallel_cases:
            # If no separate cases, just return one TestDef for each test
            return [TestDef(self.name, test_file, None) for test_file in test_list]
        ...
        for test_file in test_list:
            if test_file in self.no_parallel_cases:
                ...
                if not os.access(exe, os.X_OK):
                    ...
                    continue
                fqname = os.path.join(self.mode, self.name, test_file)
    ...
    async def create_test(self, test_def: TestDef, suite: TestSuite, args) -> None:
        allows_compaction_groups = self.all_can_run_compaction_groups_except != None and test_def.test_file not in self.all_can_run_compaction_groups_except
        test = BoostTest(self.next_id((test_def.test_file, self.suite_key, test_def.case)), test_def, suite, args, allows_compaction_groups)
        self.tests.append(test)
...
class PythonTestSuite(TestSuite):
    ...
    async def _pytest_list_cases(self, test_file: str) -> List[Optional[str]]:
        process = await asyncio.create_subprocess_exec(
        ...
        pattern = rf"{re.escape(os.path.basename(test_file))}::(\w+)"
        matches = re.findall(pattern, stdout.decode())
        assert matches, f"No test cases found in test {test_file}"
        return matches
    ...
    async def _test_defs(self, test_list: List[str]) -> List[TestDef]:
        ...
        tasks = [self._create_test_case(test_name) for test_name in test_list]
    ...
    async def add_test(self, test_def: TestDef) -> None:
        test = PythonTest(self.next_id((test_def.test_file, self.suite_key)), test_def, self)
        self.tests.append(test)
...
class CQLApprovalTestSuite(PythonTestSuite):
    ...
    async def add_test(self, test_def: TestDef) -> None:
        assert test_def.case is None
        test = CQLApprovalTest(self.next_id((test_def.test_file, self.suite_key)), test_def, self)
        self.tests.append(test)
...
class TopologyTestSuite(PythonTestSuite):
    ...
    async def add_test(self, test_def: TestDef) -> None:
        """Add test to suite"""
        test = TopologyTest(self.next_id((test_def.test_file, test_def.case, 'topology', self.mode)), test_def, self)
        self.tests.append(test)
...
class RunTestSuite(TestSuite):
    ...
    async def add_test(self, test_def: TestDef) -> None:
        assert test_def.case is None
        test = RunTest(self.next_id((test_def.test_file, test_def.case, self.suite_key)), test_def, self)
        self.tests.append(test)
...
class ToolTestSuite(TestSuite):
    ...
    async def add_test(self, test_file) -> None:
        test = ToolTest(self.next_id((test_file, self.suite_key)), test_file, self)
        self.tests.append(test)


class Test:
    """Base class for CQL, Unit and Boost tests"""
    def __init__(self, test_no: int, test_def: TestDef, suite: TestSuite) -> None:
        self.id = test_no
        self.path = ""
        self.args: List[str] = []
        self.valid_exit_codes = [0]
        # Name with test suite name
        if not isinstance(test_def.test_file, str):
            print(f"XXX Test.__init__()  {suite.name} {test_def}  test_def.test_file {type(test_def.test_file)}")
        self.path = os.path.join(suite.name, test_def.test_file)
        # Name within the suite
        self.test_def = test_def
        self.name = test_def.test_file   # XXX .test_file?
        self.case = test_def.case
        ...
        if test_def.case:
            self.uname = f"{test_def.suite_name}.{test_def.test_file}.{test_def.case}.{self.id}"
        else:
        ...
        if test_def.case is None:
            self.is_flaky = self.name in suite.flaky_tests
        else:
            self.is_flaky = f"{self.name}::{self.case}" in suite.flaky_tests
...
class UnitTest(Test):
    ...
    def __init__(self, test_no: int, test_def: TestDef, suite, args: str) -> None:
        super().__init__(test_no, test_def, suite)
        self.path = os.path.join("build", self.mode, "test", suite.name, test_def.test_file)
...
class BoostTest(UnitTest):
    ...
    def __init__(self, test_no: int, test_def: TestDef, suite, args: str,
                 allows_compaction_groups : bool) -> None:
        assert isinstance(suite, BoostTestSuite)
        boost_args = []
        if test_def.case is not None:
            boost_args += ['--run_test=' + test_def.case]
    ...
    @staticmethod
    def test_path_of_element(test: ET.Element) -> TestDef:
        path = test.attrib['path']
        elems = path.rsplit('::')
        return TestDef(elems[0], elems[1], elems[2] if len(elems) == 3 else None)
...
class CQLApprovalTest(Test):
    ...
    def __init__(self, test_no: int, test_def: TestDef, suite) -> None:
        assert test_def.case is None, "CQLApprovalTest does not have test cases inside a test"
        super().__init__(test_no, test_def, suite)
        # Path to cql_repl driver, in the given build mode
        self.path = "pytest"
        self.cql = suite.suite_path / (self.name + ".cql")
        self.result = suite.suite_path / (self.name + ".result")
        self.tmpfile = os.path.join(suite.options.tmpdir, self.mode, self.uname + ".reject")
        self.reject = suite.suite_path / (self.name + ".reject")
...
class RunTest(Test):
    ...
    def __init__(self, test_no: int, test_def: TestDef, suite) -> None:
        assert test_def.case is None, "RunTest does not support running specific test cases"
        super().__init__(test_no, test_def, suite)
        self.path = suite.suite_path / test_def.test_file
        self.xmlout = os.path.join(suite.options.tmpdir, self.mode, "xml", self.uname + ".xunit.xml")
...
class PythonTest(Test):
    ...
    def _prepare_pytest_params(self, options: argparse.Namespace):
        ...
        t = f"{str(self.suite.suite_path/self.test_def.test_file)}{'::' + self.case if self.case else ''}"
        self.args.append(t)
    ...
    async def run(self, options: argparse.Namespace) -> Test:
        ...
        try:
            ...
            if self.test_def.test_file in self.suite.dirties_cluster:
...
class ToolTest(Test):
    ...
    def __init__(self, test_no: int, test_def: str, suite) -> None:
        super().__init__(test_no, test_def, suite)
        self.path = "pytest"
        self.xmlout = os.path.join(self.suite.options.tmpdir, self.mode, "xml", self.uname + ".xunit.xml")
...
def pattern_to_case(value: str) -> TestDef:
    """From a command line test pattern create a TestDef."""
    ...
    return TestDef(suite, match.group('test'), match.group('case'))
