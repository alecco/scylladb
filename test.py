...
class TestSuite(ABC):
    ...
    def _update_run_priority(self):
        ...
        precedence = [self._mode_idx, -self.remaining_first * self._mode_idx,
                      -self.remaining * self._mode_idx]
        # Scale values by number of active workers (+1 to avoid multiply by zero)
        self.precedence = [(self.workers + 1) * x for x in precedence]
...
def parse_cmd_line() -> argparse.Namespace:
    ...
    if not args.jobs:
        if not args.cpus:
            ...
            nr_cpus = psutil.cpu_count(logical=False) * 4   # XXX bumping for CI test
        else:
