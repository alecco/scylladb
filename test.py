...
mode_run_idx = {mode: index for index, mode in enumerate(mode_run_order)}
...
class TestSuite(ABC):
    ...
    def __init__(self, path: str, cfg: dict, options: argparse.Namespace, mode: str) -> None:
        ...
        # There can be more multiple workers active on this suite
        # NOTE: if worker_lock is also needed, locking order must be worker_lock -> runner_lock
    ...
    async def run(self, runner_id: int, logger: LogPrefixAdapter, **kwargs):
        ...
        # print(f"XXX TestSuite.run: failed {failed}, done {len(done)}") # XXX
        logger.info("DONE")
...
class PythonTestSuite(TestSuite):
    ...
    async def run(self):
        ...
        result = await super().run(runner_id, logger, cluster = cluster)

        #print(f"XXX PythonTestSuite.run() {runner_id} stoping cluster")
        await cluster.stop()
        ...
        if result.failed or not self.options.save_log_on_success:
            #print(f"XXX PythonTestSuite.run() {runner_id} removing cluster")
...
async def run_all_tests(signaled: asyncio.Event, options: argparse.Namespace) -> None:
    ...
    # XXX rewrite
    # Place suites in a PriorityQueue. Worker tasks (e.g. 1 per CPU) pick suites to run.
    # There can be more than 1 worker running for a suite (executing a TestSuite.runner).
    # Suites with less/no runners are picked first as heuristic to avoid over-parallelization
    # of concurrent workers on the same suite (each runner extra resources).
    # If there are no other suites, more parallel runners are created as resources available.
    # Also workers mind memory available and required memory for a suite runner.
    ...
    class SuiteTask(NamedTuple):
        """A wrapper object to schedule suites to run with priority"""
        self.precedence: int    # priority tests - runners
        self.runners: int       # workers executing runners for this suite
        self.mode: str
        self.path: str

        def __lt__(self, other):
            # XXX  if ru
            # XXX  A-1r-2f-0r, B-0r-1f-0r, C-0r-0f-2p:  B, A, C
            # XXX  also memory requirements
            return self.suite_idx < other.suite_idx

        def __cmp__(self, other):
            # XXX
            return self.suite_idx == other.suite_idx

    # XXX  consider run first too!
    # (pending run first - runners, suite) ???
    pq = asyncio.PriorityQueue()
    worker_lock = asyncio.Lock()

    class MemoryManager:
        """Memory allocation concurrency control.
           Callers (workers) wait until there's available memory (MiB) before running.
        """
        ...
        async def reserve(self, mb_required: int):
            while True:
                async with self.lock:
                    # There might be even less available memory, double check
                    current_available = psutil.virtual_memory().available
                    if mb_required < (self.available + self.mb_reserved) and \
                            mb_required < current_available:
                        self.mb_reserved += mb_required
                        return
                    await asyncio.sleep(1)      # TODO: logarithmic backoff time
    ...
    class Worker:
        def __init__(self, name: str, memory_manager: MemoryManager,
                     task_queue: asyncio.PriorityQueue, lock: asyncio.Lock):
        ...
        async def run(self):
            ...
            while True:
                ...
                # Blocks until there's memory available
                await self.memory_manager.reserve(task.memory_required)

                # XXX work

                self.memory_manager.release(task.memory_required)
                self.task_queue.task_done()

    async def worker(name, queue, worker_lock):   # XXX , memory_lock):  ??
        logger = LogPrefixAdapter(logging.getLogger(name), {'prefix': name})

        while True:
            task = await queue.get()
            if task is None:
                queue.task_done()
                logging.info(f"{name} closing")
                break
            ...
            while True:
            async with worker_lock, task.suite.runner_lock:
                if suite.remaining == 0:
                    queue.task_done()
                    break
                await asyncio.sleep(.1)   # wait a bit before trying again
                available_memory = psutil.virtual_memory().available
                if available_memory > task.suite.memory:
                    task.suite.runners += 1    # suite runner must decrement when done (with lock)

                queue.put(task)           # XXX how to make it not the first task again??
                queue.task_done()         # let queue know *this worker* is done with the task


            # XXX put task back in? (more than 1 priority test)

            # XXX place suite again if pending > suite.runners  (run-first - runners)
    ...
    try:
        ...
        while not pq.empty():
            ...
            pending.add(asyncio.create_task(suite.run()))
            suite.runners += 1    # suite runner must decrement when done (with lock)
