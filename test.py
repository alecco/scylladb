...
class TestSuite(ABC):
    ...
    async def add_test_list(self) -> None:
        ...
        for test_name in lst:
            if not isinstance(self, BoostTestSuite):
                continue  # XXX
...
class UnitTestSuite(TestSuite):
    ...
    async def create_test(self, name, suite, args):
    ...
    async def add_test(self, name: str) -> int:
        ...
        for a in args:
            await self.create_test(name, self, a)
        return len(args)
...
class BoostTestSuite(UnitTestSuite):
    ...
    async def create_test(self, name: str, suite, args) -> None:
        ...
        if not os.access(exe, os.X_OK):
            ...
            return
        options = self.options
        allows_compaction_groups = self.all_can_run_compaction_groups_except != None and name not in self.all_can_run_compaction_groups_except
        if options.parallel_cases and (name not in self.no_parallel_cases):
            fqname = os.path.join(self.mode, self.name, name)
            ...
            if len(case_list) == 1:
                test = BoostTest(self.next_id((name, self.suite_key)), name, None, suite, args, allows_compaction_groups)
                self.tests.append(test)
            else:
                for case in case_list:
                    test = BoostTest(self.next_id((name, self.suite_key, case)), name, case, suite, args, allows_compaction_groups)
                    self.tests.append(test)
        else:
            test = BoostTest(self.next_id((name, self.suite_key)), name, None, suite, args, allows_compaction_groups)
            self.tests.append(test)
...
class PythonTestSuite(TestSuite):
    ...
    async def add_test(self, name: str) -> int:
        test = PythonTest(self.next_id((name, self.suite_key)), name, None, self)
        self.tests.append(test)
        return 1
...
class CQLApprovalTestSuite(PythonTestSuite):
    ...
    async def add_test(self, name: str) -> int:
        test = CQLApprovalTest(self.next_id((name, self.suite_key)), name, None, self)
        self.tests.append(test)
        return 1
...
class TopologyTestSuite(PythonTestSuite):
    ...
    async def add_test(self, name: str) -> int:
        """Add test to suite"""
        test = TopologyTest(self.next_id((name, 'topology', self.mode)), name, None, self)
        self.tests.append(test)
        return 1
...
class RunTestSuite(TestSuite):
    ...
    async def add_test(self, name: str) -> int:
        test = RunTest(self.next_id((name, self.suite_key)), name, None, self)
        self.tests.append(test)
        return 1
...
class Test:
    ...
    def __init__(self, test_no: int, name: str, case: Optional[str], suite: TestSuite) -> None:
        self.id = test_no
        self.path = ""
        self.args: List[str] = []
        self.valid_exit_codes = [0]
        # Name with test suite name
        self.path = os.path.join(suite.name, name)
        # Name within the suite
        self.name = name
        self.case = case
        self.full_name = f"{name}::{case}"
        self.full_name_dot = f"{name}.{case}"
...
class UnitTest(Test):
    ...
    def __init__(self, test_no: int, name: str, case: Optional[str], suite, args: str) -> None:
        super().__init__(test_no, name, case, suite)
...
class BoostTest(UnitTest):
    ...
    def __init__(self, test_no: int, name: str, case: Optional[str], suite, args: str,
                 allows_compaction_groups : bool) -> None:
        boost_args = []
        if case:
            boost_args += ['--run_test=' + case]
        super().__init__(test_no, name, case, suite, args)
...
class CQLApprovalTest(Test):
    ...
    def __init__(self, test_no: int, name: str, case: Optional[str], suite) -> None:
        assert case is None, "CQLApprovalTest does not have test cases inside a test"
        super().__init__(test_no, name, case, suite)
...
class RunTest(Test):
    ...
    def __init__(self, test_no: int, name: str, case: Optional[str], suite) -> None:
        assert case is None, "RunTest does not support running specific test cases"
        super().__init__(test_no, name, case, suite)
        self.path = suite.suite_path / name
        self.xmlout = os.path.join(suite.options.tmpdir, self.mode, "xml", self.uname + ".xunit.xml")
...
class TopologyTest(PythonTest):
    ...
    def __init__(self, test_no: int, name: str, case: Optional[str], suite) -> None:
        assert case is None, "TopologyTest does not support running by test case at the moment"
        super().__init__(test_no, name, case, suite)
