...
class TestSuite(ABC):
    ...
    def __init__(self, path: str, cfg: dict, options: argparse.Namespace, mode: str) -> None:
        ...
        # Some tests require more than the minimum memory (e.g. adding nodes to a cluster)
        self.extra_memory = collections.defaultdict(lambda: 0, self.cfg.get("extra_memory", {}))
    ...
    def _test_extra_memory(self, shortname: str) -> int:
        """Memory units used to calculate extra memory for tests"""
        return 0
    ...
    async def run(self, lock: asyncio.Lock, console: "TabularConsoleOutput", 
        ...
        while True:
            ...
            print(f"XXX TestSuite.run() {worker_name} {runner_id} {self.mode}/{self.name} test {test.shortname} extra memory {self._test_extra_memory(test.shortname)} <<<\n")
            if not await memory_manager.reserve(self._test_extra_memory(test.shortname)):
                test.is_skipped = True
                async with lock:
                    console.print_progress(test)
                    self.remaining -= 1
            ...
            try:
                for i in range(1, self.FLAKY_RETRIES):
                    if i > 1:
                        ...
                        test.reset()
                    # print(f"XXX {worker_name} {runner_id} TestSuite.run: {test.uname} 1") # XXX
                ...
                async with lock:
                    console.print_progress(test)
                await memory_manager.release(self._test_extra_memory(test.shortname))
    ...
    async def add_test_list(self) -> None:
        ...
        for test in self.tests:
            test.extra_memory = self.extra_memory[test.shortname]
...
class UnitTestSuite(TestSuite):
    ...
    def __init__(self, path: str, cfg: dict, options: argparse.Namespace, mode: str) -> None:
        ...
        self._min_memory = collections.defaultdict(lambda: 1800, {"dev": 500, "release": 1000})[self.mode]
    ...
    def _test_extra_memory(self, shortname: str) -> int:
        print(f"XXX UnitTestSuite._test_extra_memory(shortname={shortname}) <<<<<<<<<<<<<<<<\n")
        return self._min_memory * self.extra_memory[shortname]
...
class PythonTestSuite(TestSuite):
    ...
    def _test_extra_memory(self, shortname: str) -> int:
        return SCYLLA_MEMORY[self.mode] * self.extra_memory[shortname]
...
class BoostTest(UnitTest):
    ...
    def __init__(self, test_no: int, shortname: str, suite, args: str,
        ...
        boost_args = []
        print(f"XXX BoostTest(shortname={shortname}, casename={casename}")  # XXX
        if casename:
            shortname += '.' + casename
...
class MemoryManager:
    ...
    async def reserve(self, mb_required: int, check: Callable[[], bool] = lambda: True) -> bool:
        """Try to reserve memory while the condition check() is true.
           If condition check() fails, returns False.
        """
        print(f"XXX MemoryManager.reserve(mb_required={mb_required}) available {self.mb_available:.02f}, mb_reserved={self.mb_reserved} <<<")
        if mb_required == 0:
            return True
    ...
    async def release(self, memory_reserved: int):
...
async def run_all_tests(signaled: asyncio.Event, options: argparse.Namespace) -> None:
    ...
    # print(f"XXX total tests {total_tests}")
