...
fmt::print("\nXXX 1 {}.{}   prev timeuuid? {}, has current_timeuuid? {}\n\n", schema->ks_name(), schema->cf_name(), prev_timeuuid_rs->empty()? "empty" : "row", !prev_timeuuid_rs->empty()? prev_timeuuid_rs->one().has("current_timeuuid") : false); // XXX
        // XXX check ours is higher than prev
    if (!prev_timeuuid_rs->empty() && prev_timeuuid_rs->one().has("current_timeuuid")) {
//  XXX XXX XXX why nothing is there? deleted row
fmt::print("XXX 2a got previous\n\n"); // XXX
...
fmt::print("XXX 4 {}.{}\n\n", schema->ks_name(), schema->cf_name()); // XXX
...
try {
    ::shared_ptr<untyped_result_set> store_timeuuid_rs = co_await qp.execute_internal(store_timeuuid_cql,
            {tuuid, "system"});
fmt::print("\n\nXXX 5a {}.{}   store timeuuid SUCCESS {}\n", schema->ks_name(), schema->cf_name(), store_timeuuid_rs->empty()? "OK" : "got row (?)"); // XXX
} catch (std::exception& e) {
fmt::print("\n\nXXX 5b {}.{}   FAIL {}\n\n", schema->ks_name(), schema->cf_name(), e.what()); // XXX
}
