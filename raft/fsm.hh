        ...
        if constexpr (std::is_same_v<Message, append_request> ||
            ...
            if constexpr (std::is_same_v<Message, vote_request>) {
                if (current_leader() != server_id{} && election_elapsed() < ELECTION_TIMEOUT && !msg.force) {
                    ...
                    // wants to stepdown.
fmt::print(stderr, "{} [term: {}] not granting a vote (to {}) within a minimum election timeout, elapsed {} (current leader = {})\n", _my_id, _current_term, from, election_elapsed(), current_leader());
