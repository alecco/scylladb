...
// needed state machine state transitions. It produces an output
// object, which encapsulates a list of actions that must be
// performed until the next step() call can be made. The time is
// represented with a logical timer. The client is responsible for
// periodically involving tick() method, which advances the state
