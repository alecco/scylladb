...
public:
    ...
    future<> wait_until_candidate() override;
...
fmt::print("  [{}] server_impl::wait_until_candidate() wait term change\n", _id);
    while (_fsm->get_current_term() == term && _fsm->is_candidate()) {
#if 0
        co_await later();
#else
...
fmt::print("  [{}] server_impl::wait_until_candidate() DONE term {} {}\n", _id, _fsm->get_current_term(), _fsm->is_candidate()? "CANDIDATE" : "NOT CANDIDATE");
}
