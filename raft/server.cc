...
future<> server_impl::wait_for_entry(entry_id eid, wait_type type) {
    ...
    if (!inserted) {
        ...
        // This entry was dropped due to a leader change.
        auto [entry_idx, status] = std::move(*it);
        container.erase(it);
        // An entry with an older term T was replaced
        // with a new entry with term K, K > T. Could we set
        // dropped_entry exception here?
        // If we know there was at least one committed entry with
        // term L, L > T, then yes. Otherwise we must use
        // commit_status_unknown (See 3.6.2 "Committing entries
        // from previous terms" in Raft PhD: another leader with
        // another log tail may get elected before the new leader
        // commits at least one entry).
        if (*_fsm->log_term_for(_fsm->commit_idx()) > status.term) {
        ...
        if (leader == server_id{}) {
            ...
            auto reply = co_await [&] {
                if (leader == _id) {
                    ...
                    return execute_add_entry(leader, std::move(command));
                } else {
