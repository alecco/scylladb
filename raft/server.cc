...
future<add_entry_reply> server_impl::execute_modify_config(server_id from,
    ...
    try {
        ...
        for (auto& addr : add) {
            logger.trace("[{}] adding server {} as {}", id(), addr.id, addr.can_vote? "voter" : "non-voter");
            auto it = cfg.find(addr);
            if (it == cfg.end()) {
                (void) cfg.insert(addr);
            } else {
                if (it->can_vote == addr.can_vote) {
                    logger.warn("[{}] the server {} already exists in configuration as {}",
                            id(), addr.id, addr.can_vote? "voter" : "non-voter");
                } else {
                    logger.trace("[{}] making server {} {}", id(), addr.id,
