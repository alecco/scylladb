...
fmt::print("  {}  wait_log_idx_term 1: idx {} < {} ? {},  term {} < {} ? {}\n", _id, _fsm->log_last_idx(), idx_log.first, _fsm->log_last_idx() < idx_log.first, _fsm->log_last_term(), idx_log.second, _fsm->log_last_term() < idx_log.second);
    while (_fsm->log_last_idx() < idx_log.first || _fsm->log_last_term() < idx_log.second) {
fmt::print("  {}  wait_log_idx_term 2: idx {} < {} ? {},  term {} < {} ? {}\n", _id, _fsm->log_last_idx(), idx_log.first, _fsm->log_last_idx() < idx_log.first, _fsm->log_last_term(), idx_log.second, _fsm->log_last_term() < idx_log.second);
...
std::pair<index_t, term_t> server_impl::log_last_idx_term() {
    ...
    // return std::make_pair<index_t, term_t>(std::move(_fsm->log_last_idx()), _fsm->log_last_term());
}
...
fmt::print("  {} server_impl::elapse_election tick ({} < {}\n", _id, _fsm->election_elapsed(), ELECTION_TIMEOUT);
        _fsm->tick();
    }
}
