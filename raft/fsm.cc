...
fmt::print("fsm::poll_output() {} WAIT\n", _my_id); // XXX
        co_await _sm_events.wait();
fmt::print("fsm::poll_output() {} out  <<<<<<\n", _my_id); // XXX
    }
...
void fsm::tick_leader() {
    if (_clock.now() - _last_election_time >= ELECTION_TIMEOUT) {
        ...
        // with another server.
fmt::print("tick[{}]: leader becoming follower after timeout\n", _my_id); // XXX
...
fmt::print("tick[{}]: not becoming candidate\n", _my_id); // XXX
...
fmt::print("{} received a {} vote from {}\n", _my_id, reply.vote_granted ? "yes" : "no", from); // XXX
...
fmt::print("{} WON, becomming leader -------------\n", _my_id); // XXX
        become_leader();
        break;
...
bool fsm::can_read() {
    ...
    // "Stable leader" optimization ensures that followers that
    // received a communication from the current leader during the last
    // several ticks will not cast a positive vote for a new leader
    // when asked, which guarantees that if this leader got
    // replies from a quorum of followers during this tick it will
    // stay in leader role for at least a few more ticks
    // (this is a kind of a time lease).
