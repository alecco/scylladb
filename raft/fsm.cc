...
future<fsm_output> fsm::poll_output() {
    ...
    co_return make_ready_future<fsm_output>(get_output());
}
...
fsm_output fsm::get_output() {
    ...
    // Advance state machine to the new state.
    _observed.advance(*this);
    if (output.log_entries.size()) {
        ...
        move_stable_idx(output.log_entries.back()->idx);
    }

    return output;
}

void fsm::move_stable_idx(index_t idx) {
    _log.stable_to(idx);
    if (is_leader()) {
