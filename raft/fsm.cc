...
if (_my_id.id.get_least_significant_bits() < 3) fmt::print("  {} become follower new leader {}\n", _my_id, leader);
    if (leader == _my_id) {
...
void fsm::become_candidate(bool is_prevote, bool is_leadership_transfer) {
    ...
    if (!voters.contains(server_address{_my_id})) {
        ...
        // the current term.
if (_my_id.id.get_least_significant_bits() < 3) fmt::print("  {} become_candidate(): become follower\n", _my_id);
...
void fsm::tick_leader() {
    if (election_elapsed() >= ELECTION_TIMEOUT) {
        ...
        // with another server.
if (_my_id.id.get_least_significant_bits() < 3) fmt::print("  {} tick_leader timeout become follower\n", _my_id);
...
#if 0
if (!current_leader()) {
fmt::print("tick[{}]: no current leader\n", _my_id);
} else {
bool ret = current_leader() && (conf.is_joint() || conf.current.contains(server_address{current_leader()})) && _failure_detector.is_alive(current_leader());
if (!ret) fmt::print("tick[{}]: has_stable_leader: current leader {}, is joint {}, conf contains leader {}, is leader alive {}\n", _my_id, current_leader(), conf.is_joint(), conf.current.contains(server_address{current_leader()}), _failure_detector.is_alive(current_leader()));
}
#endif
...
if (_my_id.id.get_least_significant_bits() < 3) fmt::print("{} [term: {}, index: {}, last log term: {}, voted_for: {}] voted for {} [log_term: {}, log_index: {}]\n", _my_id, _current_term, _log.last_idx(), _log.last_term(), _voted_for, from, request.last_log_term, request.last_log_idx);
...
if (_my_id.id.get_least_significant_bits() < 3) fmt::print("  {} lost vote become_follower\n", _my_id);
        become_follower(server_id{});
        break;
    }
}
...
if (_my_id.id.get_least_significant_bits() < 3) fmt::print("  {} transfering leadership to id\n", _my_id, id);
        become_follower({});
    }
}
