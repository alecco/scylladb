...
void fsm::reset_election_timeout(size_t min) {
...
retries++;
    } while (candidate_timeout < min || candidate_timeout >= ELECTION_TIMEOUT.count());
...
// fmt::print("{} [term: {}, index: {}, last log term: {}, voted_for: {}] voted for {} [log_term: {}, log_index: {}] now {}\n", _my_id, _current_term, _log.last_idx(), _log.last_term(), _voted_for, from, request.last_log_term, request.last_log_idx, _clock.now());
...
// if (is_follower() && request.is_prevote) fmt::print("{} prevote for {} timeout {} elapsed {}\n", _my_id, from, _randomized_election_timeout, election_elapsed());
        if (is_follower() && request.is_prevote &&
                (election_elapsed() + logical_clock::duration{2}) >= _randomized_election_timeout) {
            // Give pre-candidate at least two ticks minimum to run it's election
            reset_election_timeout(2);
// fmt::print("\n{} new timeout {} <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n\n", _my_id, _randomized_election_timeout);
