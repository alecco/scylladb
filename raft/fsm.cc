...
void fsm::become_candidate(bool is_prevote, bool is_leadership_transfer) {
    ...
    if (!voters.contains(server_address{_my_id})) {
        ...
        // the current term.
fmt::print("{} become_candidate -> become_follower\n", _my_id);
...
void fsm::tick_leader() {
    if (_clock.now() - _last_election_time >= ELECTION_TIMEOUT) {
        ...
        // with another server.
fmt::print("{} tick_leader timeout -> become_follower\n", _my_id);
...
fmt::print("{} LOST become_follower\n", _my_id);
        become_follower(server_id{});
        break;
    }
}
...
fmt::print("{} send_timeout_now become_follower\n", _my_id);
        become_follower({});
    }
}
