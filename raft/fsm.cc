...
bool fsm::can_read() {
    ...
    // FIXME: currently this is incorrect since a leader can change immediately
    // after this one received communication from a quorum of followers. "Stable
    // leader" optimization will address this. The optimization makes sure that followers
    // that received a communication from current leader during last several ticks will not
    // cast positive vote for a new leader when asked, which will guaranty that if this leader got
