...
public:
    ...
    struct server_address_hash {
        size_t operator()(const server_address& address) const {
            return std::hash<bytes>{}(address.info);
        }
    };
    struct server_address_equal {
        bool operator()(const server_address& rhs, const server_address&lhs) const {
            return rhs.info == lhs.info;
        }
    };
    using peer_set = std::unordered_set<server_address, server_address_hash, server_address_equal>;
    struct i_am_leader {};
    struct pause {};
...
public:
    ...
    //
    discovery(server_address self, const raft::peer_list& seeds);
    ...
    // ignored.
    void response(server_address from, const peer_list& peers);
    ...
    server_id id() const { return _self.id; }
};
