...
void check_peer(const server_address& peer) {
    if (!peer.info.size()) {
        throw std::logic_error("Discovery requires peer internet address to be set");
    }
}

discovery::discovery(server_address self, const raft::peer_list& seeds)
    : _self(std::move(self)) {
...
void discovery::step(const raft::peer_list& peers) {
    ...
    for (const auto& addr : peers) {
        ...
        if (it == _peers.end() || it->id == server_id{}) {
...
peer_list discovery::request(const peer_list& peers) {
    step(peers);
    return _peer_list;
}

void discovery::response(server_address from, const peer_list& peers) {
