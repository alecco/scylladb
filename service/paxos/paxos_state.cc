...
future<prepare_response> paxos_state::prepare(tracing::trace_state_ptr tr_state, schema_ptr schema,
    ...
    return utils::get_local_injector().inject("paxos_prepare_timeout", timeout).then([&cmd, &key, ballot, tr_state, schema, only_digest, da, timeout] {
...
future<bool> paxos_state::accept(tracing::trace_state_ptr tr_state, schema_ptr schema, dht::token token, const proposal& proposal,
    ...
    return utils::get_local_injector().inject("paxos_accept_proposal_timeout", timeout).then([token = std::move(token), &proposal, schema, tr_state, timeout] {
        utils::latency_counter lc;
        lc.start();
...
future<> paxos_state::learn(schema_ptr schema, proposal decision, clock_type::time_point timeout,
    ...
    return do_with(std::move(decision), [tr_state = std::move(tr_state), schema, timeout] (proposal& decision) {
        ...
        return f.then([&decision, schema, timeout] {
            if (utils::get_local_injector().is_enabled("paxos_error_before_save_decision")) {
                return make_exception_future<>(utils::injected_error("injected_error_before_save_decision"));
            }
            ...
            auto f = db::system_keyspace::save_paxos_decision(*schema, decision, timeout);
            if (utils::get_local_injector().is_enabled("paxos_error_after_save_decision")) {
                return f.then([] {
                    return make_exception_future<>(utils::injected_error("injected_error_after_save_decision"));
                });
            } else {
                return f;
            }
