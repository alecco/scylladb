...
raft_group0::discover_leader(raft::server_address my_addr) {
    ...
    _group0 = raft::discovery{my_addr, std::move(seeds)};
    ...
    while (true) {
        ...
        if (std::holds_alternative<raft::discovery::i_am_leader>(tracker->output)) {
            co_return raft::leader_info{.addr = my_addr};
        }
...
future<raft::peer_exchange> raft_group0::peer_exchange(raft::peer_list peers) {
