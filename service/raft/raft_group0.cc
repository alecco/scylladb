...
future<raft::server_id> raft_group0::load_my_id() {
    ...
    if (!id) {
        on_internal_error(group0_log, "load_raft_addr(): server ID for group 0 missing");
    }
...
raft_group0::discover_group0(raft::server_id my_id, const std::vector<gms::inet_address>& seeds) {
    ...
    discovery_peer self = {my_id, _gossiper.get_broadcast_address()};

    auto& p_discovery = _group0.emplace<persistent_discovery>(co_await persistent_discovery::make(self, std::move(peers), _qp));
    co_return co_await futurize_invoke([this, &p_discovery, self = std::move(self)] () mutable {
        return p_discovery.run(_ms, _shutdown_gate.hold(), _abort_source, std::move(self));
...
static future<discovery::peer_list> load_discovered_peers(cql3::query_processor& qp) {
    static const auto load_cql = format(
            "SELECT ip, raft_id FROM system.{} WHERE key = '{}'",
            db::system_keyspace::GROUP0_PEERS, DISCOVERY_KEY);
    ...
    for (auto& r: *rs) {
        peers.push_back({
            ...
            gms::inet_address{r.get_as<net::inet_address>("ip")}
        });
    }
...
static mutation make_discovery_mutation(discovery::peer_list peers) {
    auto s = db::system_keyspace::group0_peers();
    ...
    for (auto& p: peers) {
        auto& row = m.partition().clustered_row(*s, clustering_key::from_singular(*s, data_value(p.ip)));
...
future<group0_info> persistent_discovery::run(
    ...
    while (true) {
        ...
        if (std::holds_alternative<discovery::pause>(output)) {
            group0_log.trace("server {} pausing discovery...", self.id);
        ...
        (void)[] (persistent_discovery& self, netw::messaging_service& ms, gate::holder pause_shutdown,
            ...
            co_await parallel_for_each(request_list, [&] (std::pair<discovery_peer, discovery::peer_list>& req) -> future<> {
                netw::msg_addr peer(req.first.ip);
...
future<> raft_group0::join_group0(std::vector<gms::inet_address> seeds, bool as_voter) {
    ...
    raft::server_id my_id = co_await load_or_create_my_id();
    group0_log.info("{} found no local group 0. Discovering...", my_id);
    while (true) {
        auto g0_info = co_await discover_group0(my_id, seeds);
        group0_log.info("server {} found group 0 with id {}, leader {}", my_id, g0_info.group0_id, g0_info.id);
        ...
        if (server->get_configuration().contains(my_id)) {
            ...
            group0_log.info("server {} already in group 0 (id {}) as {}", group0_id, my_id,
                    server->get_configuration().can_vote(my_id)? "voter" : "non-voter");
            break;
        }

        auto timeout = db::timeout_clock::now() + std::chrono::milliseconds{1000};
        netw::msg_addr peer(g0_info.ip);
        try {
            // TODO: aborts?
    ...
    _group0 = group0_id;
    group0_log.info("{} joined group 0 with id {}", my_id, group0_id);
}
...
struct group0_members {
    ...
    std::vector<gms::inet_address> get_inet_addrs() const {
        ...
        for (auto& srv: members) {
            ret.push_back(*_address_map.find(srv.addr.id));
        }
        return ret;
    }
...
future<> raft_group0::finish_setup_after_join() {
    if (joined_group0()) {
        ...
        if (!_raft_gr.group0().get_configuration().can_vote(my_id)) {
            ...
            group0_log.info("finish_setup_after_join: became a group 0 voter."); // No need to run `upgrade_to_group0()` since we must have bootstrapped with Raft
    ...
    return std::visit([this, peers = std::move(peers)] (auto&& d) mutable -> future<group0_peer_exchange> {
        ...
        if constexpr (std::is_same_v<T, std::monostate>) {
            ...
            co_return group0_peer_exchange{group0_info{
                ...
                .ip = _raft_gr.address_map().get_inet_address(_raft_gr.group0().id()),
...
future<persistent_discovery> persistent_discovery::make(discovery_peer self, peer_list seeds, cql3::query_processor& qp) {
    auto peers = co_await load_discovered_peers(qp);
    // If we're restarting discovery, the loaded peer list already
    // contains self and the old seeds. Changing the seeds between
    // restarts is not supported, so the seeds from the
    // configuration file should be ignored if the persisted list
    // is not empty.
...
future<std::optional<discovery::peer_list>> persistent_discovery::request(peer_list peers) {
    for (auto& p: peers) {
        group0_log.debug("discovery: request peer: id={}, ip={}", p.id, p.ip);
    }
...
void persistent_discovery::response(discovery_peer from, const peer_list& peers) {
    ...
    for (auto& p: peers) {
        group0_log.debug("discovery: response peer: id={}, ip={}", p.id, p.ip);
    }
...
persistent_discovery::persistent_discovery(discovery_peer self, const peer_list& seeds, cql3::query_processor& qp)
    : _discovery{std::move(self), seeds}
    , _qp{qp}
{
    for (auto& addr: seeds) {
        group0_log.debug("discovery: seed peer: id={}, info={}", addr.id, addr.ip);
    }
}
...
future<> raft_group0::do_upgrade_to_group0(group0_upgrade_state start_state) {
    ...
    if (!joined_group0()) {
        ...
        co_await join_group0(co_await sys_ks.load_peers(), true);
    } else {
        upgrade_log.info(
    ...
    co_await wait_until_every_peer_joined_group0(sys_ks, members0, _abort_source);
