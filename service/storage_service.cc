...
future<> storage_service::init_server(cql3::query_processor& qp, cql3::query_processor& qp_local,
        bind_messaging_port do_bind) {
    assert(this_shard_id() == 0);

    return seastar::async([this, &qp, &qp_local, do_bind] {
        _initialized = true;

        _group0 = std::make_unique<raft_group0>(_raft_gr, _messaging.local(), _gossiper, qp_local,
