...
BOOST_AUTO_TEST_CASE(test_confchange_a_to_b) {
    ...
    // We will not need to timeout B once we implement leader step
    // down extension.
    election_timeout(B);
    ...
    // B's new configuration is stable, but not committed yet
    // - Raft new leader never commits entries from previous terms before committing
    // an entry at new term.
    BOOST_CHECK_THROW(B.add_entry(raft::configuration({A_id})), raft::conf_change_in_progress);
    // Advance stable index, it will commit the dummy entry from the new term.
    B.get_output();
...
BOOST_AUTO_TEST_CASE(test_confchange_abcdef_to_abcgh) {
    ...
    election_timeout(H);
    communicate(C, G, H);
    BOOST_CHECK(H.is_leader());
    BOOST_CHECK_EQUAL(H.get_configuration().is_joint(), false);
    BOOST_CHECK_EQUAL(H.get_configuration().current.size(), 5);

    // H leadership is not upset
    fd.mark_all_dead();
    election_timeout(D);
    election_timeout(A);
