...
async def test_raft_upgrade_basic(manager: ManagerClient, random_tables: RandomTables):
    ...
    await asyncio.gather(*(enable_raft_and_restart(manager, srv.host_id) for srv in servers))
...
async def test_raft_upgrade_with_node_remove(manager: ManagerClient, random_tables: RandomTables):
    ...
    await enable_raft_and_restart(manager, srv1.host_id)
    ...
    await manager.server_stop_gracefully(srv1.host_id)

    logging.info(f"Enabling Raft on {others} and restarting")
    await asyncio.gather(*(enable_raft_and_restart(manager, srv.host_id) for srv in others))
    cql = await reconnect_driver(manager)
    ...
    await manager.remove_node(others[0].host_id, srv1.host_id)
...
async def test_recover_stuck_raft_upgrade(manager: ManagerClient, random_tables: RandomTables):
    ...
    await asyncio.gather(*(restart(manager, srv.host_id) for srv in others))
    cql = await reconnect_driver(manager)
...
async def test_recovery_after_majority_loss(manager: ManagerClient, random_tables: RandomTables):
    ...
    await asyncio.gather(*(manager.server_stop(srv.host_id) for srv in others))
    ...
    await restart(manager, srv1.host_id)
    cql = await reconnect_driver(manager)
    ...
    for i in range(len(others)):
        ...
        await manager.remove_node(srv1.host_id, to_remove.host_id, ignore_dead_ips)
