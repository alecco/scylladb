...
from test.pylib.manager_client import ManagerClient, HostID, IPAddress, ServerInfo
...
async def restart(manager: ManagerClient, host_id: HostID) -> None:
    logging.info(f"Stopping {host_id} gracefully")
    await manager.server_stop_gracefully(host_id)
    logging.info(f"Restarting {host_id}")
    await manager.server_start(host_id)
    logging.info(f"{host_id} restarted")


async def enable_raft(manager: ManagerClient, host_id: HostID) -> None:
    config = await manager.server_get_config(host_id)
    features = config['experimental_features']
    assert(type(features) == list)
    features.append('raft')
    logging.info(f"Updating config of server {host_id}")
    await manager.server_update_config(host_id, 'experimental_features', features)


async def enable_raft_and_restart(manager: ManagerClient, host_id: HostID) -> None:
    await enable_raft(manager, host_id)
    await restart(manager, host_id)
...
async def wait_for_cql_and_get_hosts(cql: Session, servers: list[ServerInfo], deadline: float) \
    ...
    ip_set = set(str(srv.ip) for srv in servers)
...
async def delete_raft_data(cql: Session, host: Host) -> None:
    ...
    await cql.run_async("delete value from system.scylla_local where key = 'raft_server_id'", host=host)
...
async def test_raft_upgrade_basic(manager: ManagerClient, random_tables: RandomTables):
    ...
    await asyncio.gather(*(enable_raft_and_restart(manager, srv.id_) for srv in servers))
...
async def test_raft_upgrade_with_node_remove(manager: ManagerClient, random_tables: RandomTables):
    ...
    srv1_gen = await manager.api.get_gossip_generation_number(others[0].ip, srv1.ip)
    ...
    await enable_raft_and_restart(manager, srv1.id_)
    ...
    await wait_for_gossip_gen_increase(manager.api, srv1_gen, others[0].ip, srv1.ip, time.time() + 60)

    logging.info(f"Stopping {srv1}")
    await manager.server_stop_gracefully(srv1.id_)

    logging.info(f"Enabling Raft on {others} and restarting")
    await asyncio.gather(*(enable_raft_and_restart(manager, srv.id_) for srv in others))
    cql = await reconnect_driver(manager)
    ...
    await manager.remove_node(others[0].id_, srv1.id_)
...
async def test_recover_stuck_raft_upgrade(manager: ManagerClient, random_tables: RandomTables):
    ...
    async with inject_error(manager.api, srv1.ip, 'group0_upgrade_before_synchronize',
                            one_shot=True):
        logging.info(f"Enabling Raft on {others} and restarting")
        await asyncio.gather(*(enable_raft_and_restart(manager, srv.id_) for srv in others))
        cql = await reconnect_driver(manager)
    ...
    await asyncio.gather(*(restart(manager, srv.id_) for srv in others))
    cql = await reconnect_driver(manager)
...
async def test_recovery_after_majority_loss(manager: ManagerClient, random_tables: RandomTables):
    ...
    await asyncio.gather(*(manager.server_stop(srv.id_) for srv in others))

    logging.info(f"Entering recovery state on {srv1}")
    host1 = next(h for h in hosts if h.address == srv1.ip)
    await cql.run_async("update system.scylla_local set value = 'recovery' where key = 'group0_upgrade_state'", host=host1)
    await restart(manager, srv1.id_)
    cql = await reconnect_driver(manager)

    logging.info("Node restarted, waiting until driver connects")
    host1 = (await wait_for_cql_and_get_hosts(cql, [srv1.ip], time.time() + 60))[0]

    for i in range(len(others)):
        to_remove = others[i]
        ignore_dead_ips = [srv.ip for srv in others[i+1:]]
        logging.info(f"Removing {to_remove} using {srv1} with ignore_dead: {ignore_dead_ips}")
        await manager.remove_node(srv1.id_, to_remove.id_, ignore_dead_ips)
