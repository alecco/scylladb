...
def pytest_addoption(parser):
    ...
    parser.addoption('--cluster_api_sock', action='store', required=True,
...
async def cluster_api(request):
    sock_path = request.config.getoption('cluster_api_sock')
...
async def cql(request, cluster_api):
    ...
    cql_port = await cluster_api.cql_port()
...
@pytest.fixture(scope="session")
async def test_keyspace(cql, this_dc, cluster_api):
    name = unique_name()
