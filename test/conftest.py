...
async def harness_internal(request):
    ...
    harness_int = HarnessCli(sock_path, cluster_con_ssl)
    yield harness_int
    print(f"XXX {request.node.name} fixture harness INTERNAL going to stop driver", file=stderr)  # XXX
    # Close driver connection and tell Harness server we are done
    harness_int._driver_close()
...
async def random_tables(request, harness, cql, keyspace) -> AsyncGenerator:
    ...
    # XXX await tables.drop_all_tables()
