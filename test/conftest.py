...
async def random_tables(request, cql, test_keyspace) -> AsyncGenerator:
    """Provides a RandomTables helper for schema changes.
       Automatically drops all tables after test is done"""
    tables = RandomTables(request.node.name, cql, test_keyspace)
    yield tables
