...
from test.pylib.schema_helper import TestTables, create_keyspace         # type: ignore
...
@pytest.fixture(scope="session")
async def keyspace(request, cql, this_dc) -> AsyncGenerator:
    name = unique_name()
...
async def tables(request, cql, keyspace) -> AsyncGenerator:
    ...
    tables = TestTables(request.node.name, cql, await create_keyspace(cql))
    return tables
