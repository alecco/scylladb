...
class ScyllaCluster:
    ...
    Control is provided through an HTTP REST API through an AF_UNIX socket.
        ...
        /cql-port - CQL port number used in this cluster
    ...
    @property
    def is_running(self) -> bool:
        return self.cmd is not None  # XXX
    ...
    async def uninstall(self) -> None:
        """Clear all files left from a stopped cluster"""

        # XXX
        if not self.hostname:
            return
        logging.info("Uninstalling cluster at %s", self.workdir)
    ...
    async def cluster_stop(self, request) -> aiohttp.web.Response:
        ...
        self.last_seed = None
        self.removed.update(self.started)
        self.started.clear()
        return aiohttp.web.Response(text="OK")
