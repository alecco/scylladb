...
from test.pylib.manager_client import ServerID, IPAddress, HostID, ServerInfo
import aiohttp
import aiohttp.web
import yaml
import signal
...
class ScyllaServer:
    ...
    def __init__(self, exe: str, vardir: str,
        ...
        self.server_id = ServerID(ScyllaServer.newid())
    ...
    def __str__(self):
        ...
        return f"ScyllaServer({self.server_id}, {host_ip}, {host_id})"
...
class ScyllaCluster:
    ...
    def __init__(self, replicas: int,
        ...
        self.running: Dict[ServerID, ScyllaServer] = {}        # started servers
        self.stopped: Dict[ServerID, ScyllaServer] = {}        # servers no longer running but present
        self.servers = ChainMap(self.running, self.stopped)
        self.decommissioned: Dict[ServerID, ScyllaServer] = {} # decommissioned servers
        self.removed: Set[ServerID] = set()                    # removed servers (might be running)
    ...
    async def add_server(self) -> ServerInfo:
        ...
        return ServerInfo(server.server_id, server.host_ip)
    ...
    def running_servers(self) -> List[Tuple[ServerID, IPAddress]]:
        """Get a list of host ids and IP address of running servers (and not removed)"""
        return [(server.server_id, server.host_ip) for server in self.running.values()
    ...
    async def server_stop(self, server_id: ServerID, gracefully: bool) -> ActionReturn:
    ...
    def server_mark_decommissioned(self, server_id: ServerID) -> None:
    ...
    async def server_restart(self, server_id: ServerID) -> ActionReturn:
    ...
    def get_config(self, server_id: ServerID) -> ActionReturn:
    ...
    def update_config(self, server_id: ServerID, key: str, value: object) -> ActionReturn:
...
class ScyllaClusterManager:
    ...
    def _setup_routes(self, app: aiohttp.web.Application) -> None:
        ...
        app.router.add_get('/cluster/host-ip/{server_id}', self._cluster_host_ip)
    ...
    async def _cluster_host_ip(self, request) -> aiohttp.web.Response:
        """IP address of a server"""
        server_id = ServerID(int(request.match_info["server_id"]))
        return aiohttp.web.Response(text=f"{self.cluster.servers[server_id].host_ip}")
    ...
    async def _cluster_remove_node(self, request: aiohttp.web.Request) -> aiohttp.web.Response:
        ...
        initiator_id = ServerID(int(request.match_info["initiator"]))
        server_id = ServerID(int(data["server_id"]))
        assert isinstance(data["ignore_dead"], list), "Invalid list of dead IP addresses"
        ignore_dead = [IPAddress(h_ip) for h_ip in data["ignore_dead"]]
        ...
        # initate remove
        try:
            await self.cluster.api.remove_node(initiator.host_ip, to_remove.host_id, ignore_dead)
    ...
    async def _server_get_config(self, request: aiohttp.web.Request) -> aiohttp.web.Response:
        """Get conf/scylla.yaml of the given server as a dictionary."""
        assert self.cluster
        ret = self.cluster.get_config(ServerID(int(request.match_info["server_id"])))
        if not ret.success:
    ...
    async def _server_update_config(self, request: aiohttp.web.Request) -> aiohttp.web.Response:
        ...
        assert self.cluster
        data = await request.json()
        ret = self.cluster.update_config(ServerID(int(request.match_info["server_id"])),
                                         data['key'], data['value'])
        if not ret.success:
            return aiohttp.web.Response(status=404, text=ret.msg)
