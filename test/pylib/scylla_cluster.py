...
class ScyllaServer:
    ...
    async def stop(self) -> None:
        ...
        logging.info("stopping server %s in %s", self, self.workdir.name)
        if not self.cmd:
            return
        ...
        try:
            ...
            if self.cmd:
                logging.info("stopped server %s in %s", self, self.workdir.name)
            self.cmd = None
    ...
    async def stop_gracefully(self) -> None:
        ...
        try:
            ...
            if self.cmd:
                logging.info("gracefully stopped server %s", self)
            self.cmd = None
    ...
    async def uninstall(self) -> None:
        ...
        if not hasattr(self, "host_ip"):
            logging.warning("Trying to uninstall of server %s not set up", self)
            return
...
class ScyllaClusterManager:
    ...
    async def _cluster_server_add(self, _request) -> aiohttp.web.Response:
        ...
        return aiohttp.web.json_response({"host_id" : s_info.host_id, "host_ip": s_info.host_ip})
