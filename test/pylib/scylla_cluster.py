...
class ScyllaCluster:
    ...
    async def stop(self) -> None:
        ...
        logging.info(f"Cluster %s stopping", self)
        if self.is_running:
    ...
    async def stop_gracefully(self) -> None:
        ...
        logging.info(f"Cluster %s stopping gracefully", self)
        self.dirty = True
        if self.is_running:
    ...
    async def add_server(self, mark_dirty:bool=True) -> str:
        ...
        try:
            logging.info(f"Cluster %s adding server", self)
    ...
    # XXX REMOVE DEBUG
    def _eraseme_ks(self) -> None:
        """Get the current keyspace count"""
        assert self[0].control_connection is not None
        rows = self[0].control_connection.execute(
            "select keyspace_name from system_schema.keyspaces LIMIT 100")
        print(f" >>> keyspaces:\n {rows}\n", file=stderr) # XXX
    ...
    async def node_stop(self, node_id: str, gracefully: bool) -> bool:
        ...
        self.dirty = True
        logging.info(f"Cluster %s stopping server %s", self, node_id)
    ...
    async def node_start(self, node_id: str) -> bool:
        ...
        self.dirty = True
        logging.info(f"Cluster %s starting server", self)
    ...
    async def node_restart(self, node_id: str) -> bool:
        ...
        self.dirty = True
        logging.info(f"Cluster %s restarting server %s", self, node_id)
        server = self.started.pop(node_id, None)
        if server is None:
            return False
        await server.stop_gracefully()
        await server.start()
    ...
    async def node_remove(self, node_id: str) -> bool:
        ...
        self.dirty = True
        logging.info(f"Cluster %s removing server %s", self, node_id)
    ...
    async def node_replace(self, old_node_id) -> Optional[str]:
        ...
        self.dirty = True
        logging.info(f"Cluster %s replacing server %s", self, node_id)
...
class Harness:
    ...
    async def _before_test(self, request) -> aiohttp.web.Response:
        ...
        logging.info(f"Leasing Scylla cluster %s for test %s", self.cluster, test_name)
        return aiohttp.web.Response(text="True")
