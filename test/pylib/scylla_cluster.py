...
class ScyllaCluster:
    def __init__(self, scylla_exe: str, replicas: int, test_base_dir: str,
                 cmdline_options: List[str], config_options: Dict[str, str],
                 host_registry: HostRegistry, artifact_registry: ArtifactRegistry) -> None:
    ...
    def __getitem__(self, i: int) -> ScyllaServer:
        ...
        return list(self.started.values())[i]

    def __str__(self):
        path = pathlib.PurePath(self.cluster_dir)
        return f"{{{path.name}: {', '.join(str(c) for c in self.started)}}}"
...
class Harness:
    ...
    def __init__(self, test_name: str, base_dir: str, clusters: Pool[ScyllaCluster]) -> None:
        ...
        self.harness_dir: str = tempfile.mkdtemp(prefix="harness-", dir=base_dir)
