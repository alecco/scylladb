...
class RandomTable():
    ...
    async def add_column(self, name: str = None, ctype: Type[ValueType] = None, column: Column = None):
        ...
        await self.manager.cql.run_async(f"ALTER TABLE {self.full_name} ADD {column.name} {column.ctype.name}")
...
class RandomTables():
    ...
    def __init__(self, test_name: str, manager: ManagerClient, keyspace: str):
        ...
        manager.cql.execute(f"CREATE KEYSPACE {keyspace} WITH REPLICATION = "
    ...
    async def drop_all(self) -> None:
        ...
        await self.manager.cql.run_async(f"DROP KEYSPACE {self.keyspace}")
