...
class RandomTable():
    ...
    async def create(self) -> asyncio.Future:
        ...
        assert self.m.cql is not None
        return await self.m.cql.run_async(cql_stmt)
    ...
    async def add_column(self, name: str = None, ctype: Type[ValueType] = None, column: Column = None):
        ...
        assert self.m.cql is not None
        await self.m.cql.run_async(f"ALTER TABLE {self.full_name} " 
    ...
    async def drop_column(self, column: Union[Column, str] = None):
        ...
        assert self.m.cql is not None
        await self.m.cql.run_async(f"ALTER TABLE {self.full_name} DROP {col.name}")

    async def insert_seq(self) -> asyncio.Future:
        """Insert a row of next sequential values"""
        seed = self.next_seq()
        assert self.m.cql is not None
        return await self.m.cql.run_async(f"INSERT INTO {self.full_name} ({self.all_col_names})"
    ...
    async def add_index(self, column: Union[Column, str], name: str = None) -> str:
        ...
        assert self.m.cql is not None
        await self.m.cql.run_async(f"CREATE INDEX {name} on {self.full_name} ({col_name})")
    ...
    async def drop_index(self, name: str) -> None:
        ...
        assert self.m.cql is not None
        await self.m.cql.run_async(f"DROP INDEX {self.keyspace}.{name}")
...
class RandomTables():
    ...
    def __init__(self, test_name: str, manager: ManagerClient, keyspace: str):
        ...
        assert self.m.cql is not None
        self.m.cql.execute(f"CREATE KEYSPACE {keyspace} WITH REPLICATION = "
    ...
    async def add_tables(self, ntables: int = 1, ncolumns: int = 5) -> None:
        ...
        tables = [RandomTable(self.m, self.keyspace, ncolumns) for _ in range(ntables)]
    ...
    async def add_table(self, ncolumns: int = None, columns: List[Column] = None,
        ...
        table = RandomTable(self.m, self.keyspace, ncolumns=ncolumns, columns=columns,
                            pks=pks, name=name)
        await table.create()
    ...
    async def drop_all(self) -> None:
        ...
        assert self.m.cql is not None
        await self.m.cql.run_async(f"DROP KEYSPACE {self.keyspace}")
    ...
    async def verify_schema(self, table: Union[RandomTable, str] = None) -> None:
        ...
        assert self.m.cql is not None
        res1 = {row.table_name for row in await self.m.cql.run_async(cql_stmt1)}
        ...
        for table_name in tables:
            ...
            assert self.m.cql is not None
            res2 = {row.column_name: row for row in await self.m.cql.run_async(cql_stmt2)}
