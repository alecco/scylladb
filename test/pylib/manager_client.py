...
class ManagerClient():
    ...
    async def remove_node(self, initiator_id: ServerID, host_id: ServerID,
    ...
    async def decommission_node(self, host_id: ServerID) -> None:
    ...
    async def server_get_config(self, host_id: ServerID) -> dict[str, object]:
        resp = await self.client.get(f"/cluster/server/{host_id}/get_config")
        if resp.status != 200:
            raise Exception(await resp.text())
        return await resp.json()

    async def server_update_config(self, host_id: ServerID, key: str, value: object) -> None:
    ...
    async def get_host_ip(self, server_id: ServerID) -> IPAddress:
        ...
        resp = await self.client.get(f"/cluster/host-id/{server_id}")
        if resp.status != 200:
            raise Exception(f"Failed to get IP address of server {server_id} {await resp.text()}")
        return IPAddress(await resp.text())

    async def get_host_id(self, server_id: ServerID) -> HostID:
        """Get host id through Scylla REST API"""
        resp = await self.client.get(f"/cluster/host-id/{server_id}")
        if resp.status != 200:
            raise Exception(f"Failed to get host ID of server {server_id} {await resp.text()}")
        return HostID(await resp.text())
