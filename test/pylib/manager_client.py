...
class ManagerClient():
    ...
    async def driver_connect(self) -> None:
        ...
        if self.con_gen is not None:
            servers = await self.active_servers()
    ...
    async def before_test(self, test_case_name: str) -> None:
        ...
        resp = await self.cli.get(f"/cluster/before-test/{test_case_name}")
        if resp.status != 200:
    ...
    async def after_test(self, test_case_name: str) -> None:
        ...
        await self.cli.get(f"/cluster/after-test")
    ...
    async def running_servers(self) -> List[str]:
        """Get list of running servers (including decommissioned)"""
        host_list = await self.cli.get_text("/cluster/running-servers")
        return host_list.split(",")

    async def active_servers(self) -> List[str]:
        """Get list of active servers (running and not decommissioned)"""
        host_list = await self.cli.get_text("/cluster/active-servers")
        return host_list.split(",")
