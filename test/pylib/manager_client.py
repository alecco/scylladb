...
class ManagerClient():
    ...
    async def running_servers(self) -> List[ServerInfo]:
        ...
        return [ServerInfo(ServerNum(int(info[0])), info[1]) for info in server_info_list]
    ...
    async def server_add(self) -> ServerInfo:
        ...
        try:
            s_info = ServerInfo(ServerNum(int(server_info["server_id"])), server_info["ip_addr"])
        except Exception as exc:
    ...
    async def get_host_id(self, server_id: ServerNum) -> HostID:
        """Get given server's host id (UUID)"""
        try:
