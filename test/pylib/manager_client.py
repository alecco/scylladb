...
class ManagerClient():
    ...
    async def server_start(self, server_id: ServerNum, expected_error: Optional[str] = None,
        ...
        await self.server_sees_others(server_id, wait_others)
        self._driver_update()

    async def server_restart(self, server_id: ServerNum, wait_others: int = 0) -> None:
    ...
    async def server_sees_others(self, server_id: ServerNum, count: int, deadline: int = 45):
        ...
        async def _sees_min_others():
            ...
            if len(alive_nodes) > count:
                return True
        await wait_for(_sees_min_others, time() + deadline, period=.1)
    ...
    async def server_sees_other_server(self, server_ip: IPAddress, other_ip: IPAddress,
        ...
        async def _sees_another_server():
            ...
            if other_ip in alive_nodes:
                return True
        await wait_for(_sees_another_server, time() + deadline, period=.1)
    ...
    async def server_not_sees_other_server(self, server_ip: IPAddress, other_ip: IPAddress):
        ...
        async def _not_sees_another_server():
            ...
            if not other_ip in alive_nodes:
                return True
        await wait_for(_not_sees_another_server, time() + 45, period=.1)
