...
def _wrap_future(driver_response_future: ResponseFuture, first_page_only: bool = True) -> asyncio.Future:
    """Wrap a cassandra Future into an asyncio.Future object.

    Args:
        driver_response_future: future to wrap
        first_page_only: only return with first page of results
    ...
    def on_result(result):
        if aio_future.done():
            ...
            if result is None:
                ...
                if not driver_response_future.has_more_pages or first_page_only:
...
def run_async(self, *args, first_page_only = True, **kwargs) -> asyncio.Future:
    ...
    return _wrap_future(self.execute_async(*args, **kwargs), first_page_only = first_page_only)
