...
class HarnessCli():
    ...
    async def _driver_connect(self) -> None:
        # print("XXX _driver_connect() driver connecting...", file=stderr)  # XXX
        self.ccluster = self.con_gen(await self.nodes(), await self.cql_port())
        self.cql = self.ccluster.connect()
        # print("XXX _driver_connect() connected.", file=stderr)  # XXX

    # XXX is this needed? does this make the test less realistic?
    ...
    def _driver_update_and_mark_dirty(self) -> None:
        self.cql.refresh_node_list_and_token_map()
        self.dirty = True
    ...
    async def before_test(self, test_name: str) -> None:
        ...
        # print(f"XXX before test {test_name}", file=stderr)  # XXX
        if await self.is_dirty():
            # print(f"   cluster dirty, closing driver", file=stderr)  # XXX
    ...
    async def after_test(self, test_name: str) -> None:
        ...
        # print(f"XXX after test {test_name}", file=stderr)  # XXX
