...
int dummy_command = std::numeric_limits<int>::min();
...
future<> raft_cluster::state_machine::apply(const std::vector<raft::command_cref> commands) {
...
future<raft::snapshot_id> raft_cluster::state_machine::take_snapshot() {
...
void raft_cluster::state_machine::drop_snapshot(raft::snapshot_id id) {
    (*_snapshots).erase(_id);
}

future<> raft_cluster::state_machine::load_snapshot(raft::snapshot_id id) {
...
future<> raft_cluster::state_machine::abort() { return make_ready_future<>(); }

future<> raft_cluster::state_machine::done() {
    return _done.get_future();
}

future<raft::snapshot_reply> raft_cluster::rpc::send_snapshot(raft::server_id id, const raft::install_snapshot& snap, seastar::abort_source& as) {
    if (!net.count(id)) {
...
future<> raft_cluster::rpc::send_append_entries(raft::server_id id, const raft::append_request& append_request) {
    if (!net.count(id)) {
...
void raft_cluster::rpc::send_append_entries_reply(raft::server_id id, const raft::append_reply& reply) {
    if (!net.count(id)) {
        return;
    }
    if (!(*_connected)(id, _id)) {
...
void raft_cluster::rpc::send_vote_request(raft::server_id id, const raft::vote_request& vote_request) {
    if (!net.count(id)) {
        return;
    }
    if (!(*_connected)(id, _id)) {
...
void raft_cluster::rpc::send_vote_reply(raft::server_id id, const raft::vote_reply& vote_reply) {
    if (!net.count(id)) {
        return;
    }
    if (!(*_connected)(id, _id)) {
...
void raft_cluster::rpc::send_timeout_now(raft::server_id id, const raft::timeout_now& timeout_now) {
    if (!net.count(id)) {
        return;
    }
    if (!(*_connected)(id, _id)) {
...
void raft_cluster::rpc::add_server(raft::server_id id, bytes node_info) {
    _known_peers.insert(raft::server_address{id});
    ++_servers_added;
}

void raft_cluster::rpc::remove_server(raft::server_id id) {
    _known_peers.erase(raft::server_address{id});
    ++_servers_removed;
}

future<> raft_cluster::rpc::abort() {
    return make_ready_future<>();
}

void raft_cluster::rpc::reset_network() {
    net.clear();
}

const raft::server_address_set& raft_cluster::rpc::known_peers() const {
    return _known_peers;
}

void raft_cluster::rpc::reset_counters() {
    _servers_added = 0;
    _servers_removed = 0;
}

uint32_t raft_cluster::rpc::servers_added() const {
    return _servers_added;
}

uint32_t raft_cluster::rpc::servers_removed() const {
