...
fmt::print("{}: apply_changes res {}/{}\n", id, res.size(), expected); // XXX
...
fmt::print("Adding entry on leader 000[{}]\n", leader + 1);
...
fmt::print("Calling elect me leader on 000[{}]\n", next_leader + 1);
...
int main(int argc, char* argv[]) {
    ...
    std::vector<test_case> replication_tests = {
        ...
        {.name = "simple_3_spurious", .nodes = 3, .initial_term = 2, .initial_leader = 0,
         ...
         .updates = {entries{3,4}},},
#endif
        // XXX this test fails on third election when changing leader to {2}
        //     unless there's a 1us sleep or read barrier before election triggered
        // 3 nodes, term 2, follower has spurious entry, multiple leader changes
#if 0
        {.name = "simple_3_spurious_multi_leaders", .nodes = 3, .initial_term = 3, .initial_leader = 0,
         .initial_states = {{},
                            {.le = {{1,10},{2,20}}, .start_idx = 1}},
