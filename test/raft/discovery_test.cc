...
using discovery_network = std::unordered_map<server_id, discovery*>;
...
run_discovery_impl(discovery_network& network) {
    while (true) {
        for (auto e : network) {
            ...
            if (std::holds_alternative<raft::discovery::i_am_leader>(output)) {
                return;
            } else if (std::holds_alternative<raft::discovery::pause>(output)) {
                continue;
            }
            auto& msgs = std::get<raft::discovery::request_list>(output);
            for (auto&& m : msgs) {
