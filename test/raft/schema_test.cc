...
SEASTAR_TEST_CASE(test_create_table_updates_timestampuuid) {
    return do_with_cql_env([] (cql_test_env& env) -> future<> {
        ...
        auto create_cf_get_tu = [&] (const sstring& cf) -> future<res_pair> {
        ...
        auto res_1 = co_await create_cf_get_tu("t1");
        auto res_2 = co_await create_cf_get_tu("t2");
        auto res_3 = co_await create_cf_get_tu("t3");
