...
class raft_cluster {
    ...
    static std::uniform_int_distribution<int> _rpc_extra_delay_dist;
...
public:
    rpc(raft::server_id id, connected* connected, snapshots* snapshots,
            rpc_config rpc_config) : _id(id), _connected(connected), _snapshots(snapshots),
    ...
    virtual void send_read_quorum(raft::server_id id, const raft::read_quorum& read_quorum) {
        ...
        if (!drop_packet()) {
            net[id]->_client->read_quorum_request(_id, read_quorum);
        }
    }
    virtual void send_read_quorum_reply(raft::server_id id, const raft::read_quorum_reply& reply) {
        ...
        if (!drop_packet()) {
            net[id]->_client->read_quorum_reply(_id, std::move(reply));
        }
    }
    virtual future<raft::read_barrier_reply> execute_read_barrier_on_leader(raft::server_id id) {
        ...
        return net[id]->_client->execute_read_barrier(_id);
    }
    ...
    virtual future<raft::add_entry_reply> send_add_entry(raft::server_id id, const raft::command& cmd) override {
        ...
        return net[id]->_client->execute_add_entry(_id, cmd);
    }
    virtual future<raft::add_entry_reply> send_modify_config(raft::server_id id,
        ...
        check_known_and_connected(id);
        return net[id]->_client->execute_modify_config(_id, add, del);
    }
...
public:
    rpc_with_delays(raft::server_id id, connected* connected, snapshots* snapshots,
            ...
            rpc(id, connected, snapshots, rpc_config),
            _dist(dist), _gen(gen)
    {
        rpc::net[rpc::_id] = this;
        // Rounds to next power of 2
        _local_mask = std::bit_width(_rpc_config.local_nodes);
    }
    ...
    virtual future<> send_append_entries(raft::server_id id, const raft::append_request& append_request) {
        ...
        if (!rpc::_rpc_config.drops || (rand() % 5)) {
...
template <typename Clock>
std::uniform_int_distribution<int> raft_cluster<Clock>::_rpc_extra_delay_dist;
