...
public:
            ...
            , _delays(rpc_config.network_delay > 0ms)
    {
        ...
        _local_mask = (1 << std::bit_width(_rpc_config.local_nodes)) - 1;
    }
    bool drop_packet() {
        return _rpc_config.drops && !(rand() % 5);
    }
    bool is_local_node(raft::server_id& id) {
        return (to_int_id(_id.id) & _local_mask) == (to_int_id(id.id) & _local_mask);
    }
...
typename raft_cluster<Clock>::test_server raft_cluster<Clock>::create_server(size_t id, initial_state state) {
    ...
    // 'unique_ptr<raft_cluster<seastar::lowres_clock>::rpc,
    //          default_delete<raft_cluster<seastar::lowres_clock>::rpc>>'
    // to
    // 'unique_ptr<raft::rpc, default_delete<raft::rpc>>'

    std::unique_ptr<raft_cluster::rpc> mrpc = std::make_unique<raft_cluster::rpc>(uuid, _connected.get(),
            _snapshots.get(), _rpc_config, _rpc_extra_delay_dist,
            _rpc_extra_delay_gen);
    auto& rpc_ref = *mrpc;
