...
template <typename State>
class persistence_wrapper : public raft::persistence {
    snapshots_t<State>& _snapshots;
...
public:
    ...
    // Otherwise it must be initialized with an empty configuration (it will be added to the cluster
    // through a configuration change) and `init_config_id` must be `nullopt`.
    persistence_wrapper(snapshots_t<State>& snaps, std::optional<raft::server_id> init_config_id, State init_state)
        : _snapshots(snaps)
...
public:
    ...
    static std::unique_ptr<raft_server> create(
        ...
        auto persistence_ = std::make_unique<persistence_wrapper<state_t>>(*snapshots, first_server ? std::optional{id} : std::nullopt, M::init);
