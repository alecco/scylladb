...
public:
    persistence_wrapper(snapshots_t<State>& snaps, lw_shared_ptr<::persistence<State>> persistence)
        : _snapshots(snaps)
...
public:
    ...
    static std::unique_ptr<raft_server> create(
        ...
        auto persistence_ = std::make_unique<persistence_wrapper<state_t>>(*snapshots, std::move(persistence));
