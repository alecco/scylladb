...
public:
    ...
    static future<> do_with(std::function<future<>(cql_test_env&)> func, cql_test_config cfg_in) {
        ...
        return seastar::async([cfg_in = std::move(cfg_in), func] {
            ...
            proxy.start(std::ref(db), std::ref(gossiper), spcfg, std::ref(b), scheduling_group_key_create(sg_conf).get0(), std::ref(feature_service), std::ref(token_metadata), std::ref(ms), false).get();
            auto stop_proxy = defer([&proxy] { proxy.stop().get(); });

            sharded<service::storage_proxy> proxy_local;
            proxy_local.start(std::ref(db), std::ref(gossiper), spcfg, std::ref(b),
                scheduling_group_key_create(sg_conf).get0(), std::ref(feature_service),
                std::ref(token_metadata), std::ref(ms), true).get();
            ...
            ss.local().init_server(qp_local.local()).get();
            try {
