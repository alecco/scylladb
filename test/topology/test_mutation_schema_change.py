...
async def test_mutation_schema_change(manager, random_tables):
    ...
    async def server_c_sees_another_server():
        alive_nodes = await manager.api.get_alive_endpoints(server_c.ip_addr)
        if len(alive_nodes) > 1:
            return True
    await wait_for(server_c_sees_another_server, time.time() + 45, period=.1)
