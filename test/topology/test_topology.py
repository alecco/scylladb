...
@pytest.mark.asyncio
async def test_add_node_add_column(cql, cluster_api, random_tables):
...
@pytest.mark.asyncio
async def test_stop_node_add_column(cql, cluster_api, random_tables):
...
@pytest.mark.asyncio
async def test_restart_node_add_column(cql, cluster_api, random_tables):
    """Add a node, stop an original node, add a column"""
    nodes = await cluster_api.nodes()
    table = await random_tables.add_table(ncolumns=5)
...
@pytest.mark.asyncio
async def test_remove_node_add_column(cql, cluster_api, random_tables):
...
@pytest.mark.asyncio
async def test_decomission_node_add_column(cql, cluster_api, random_tables):
...
@pytest.mark.asyncio
async def test_replace_node_add_column(cql, cluster_api, random_tables):
