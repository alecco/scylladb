...
async def test_stop_server_add_column(manager, random_tables):
    ...
    table = await random_tables.add_table(ncolumns=5)
    await manager.server_add()
    await manager.server_stop(servers[1].host_id)
    await table.add_column()
    await random_tables.verify_schema()
...
async def test_restart_server_add_column(manager, random_tables):
    ...
    servers = await manager.running_servers()
    table = await random_tables.add_table(ncolumns=5)
    ret = await manager.server_restart(servers[1].host_id)
    await table.add_column()
    await random_tables.verify_schema()
...
async def test_remove_node_add_column(manager, random_tables):
    ...
    table = await random_tables.add_table(ncolumns=5)
    await manager.server_add()
    await manager.server_stop_gracefully(servers[1].host_id)            # stop     [1]
    await manager.remove_node(servers[0].host_id, servers[1].host_id)   # Remove   [1]
    await table.add_column()
    await random_tables.verify_schema()
...
async def test_decommission_node_add_column(manager, random_tables):
    ...
    table = await random_tables.add_table(ncolumns=5)
    await manager.server_add()
    await manager.decommission_node(servers[1].host_id)             # Decommission [1]
    await table.add_column()
    await random_tables.verify_schema()
