...
async def test_delete_empty_string_key(cql, keyspace):
    ...
    await cql.run_async(f"DELETE FROM {keyspace.name}.{keyspace.tables[0]} WHERE p='{s}' AND c=''")
    ...
    with pytest.raises(InvalidRequest, match='Key may not be empty'):
        await cql.run_async(f"DELETE FROM {keyspace.name}.{keyspace.tables[0]} WHERE p='' AND c='{s}'")
