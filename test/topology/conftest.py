...
class Table():
    ...
    async def add_index(self, column, name=None):
        ...
        print(f"XXX CREATE INDEX {name} on {self.full_name} ({col.name})", file=sys.stderr)  # XXX
    ...
    async def drop_index(self, index):
        ...
        print(f"XXX DROP INDEX {self.keyspace}.{index}", file=sys.stderr) # XXX
...
class TestTables():
    ...
    async def verify_schema(self, table=None):
        ...
        for table_name in tables:
            ...
            assert res.keys() == cols.keys(), f"Column names do not match {res.keys()} {cols.keys()}"
