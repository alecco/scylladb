...
static int32_t paxos_ttl_sec(const schema& s) {
    // Keep paxos state around for gc_grace_seconds. If one of the Paxos participants
    ...
    return std::chrono::duration_cast<std::chrono::seconds>(s.gc_grace_seconds()).count();
}
