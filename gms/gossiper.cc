...
future<std::set<inet_address>> gossiper::get_live_members_synchronized(std::chrono::milliseconds timeout) {
    ...
    size_t nr_version = co_await container().map_reduce0([version, timeout] (gossiper& local_gossiper) -> future<size_t> {
        try {
            co_await local_gossiper._live_endpoints_version_condvar.when(timeout, [version, &local_gossiper] {
                 return local_gossiper._live_endpoints_version >= version;
            });
        } catch (...) {
            logger.error("Shard {} timed out waiting for gossiper version {}", this_shard_id(), version);
            co_return 0;
        }
        co_return 1;
    }, 0, std::plus<size_t>());
    logger.debug("Shards with gossiper version >= {}: {} / {}", version, nr_version, smp::count);
    if (nr_version < smp::count) {
        throw std::runtime_error(format("Failed wait for gossip version on all shards"));
    }
